...
This poll allows the MakerDAO governance community to signal their support or opposition to adding CASTLEDAO-A (Gelato DAI-USDC) as a new vault type in the Maker Protocol with the parameters below:
...
*Liquidations are disabled for this collateral type. The risk for CASTLEDAO-A is primarily from USDC tail risks, and so it's impossible to generate appropriate risk parameters from previous data.*
...
Please review the following forum threads containing information about CASTLEDAO-A (Gelato) to inform your position before voting.
...
* CASTLEDAO-A (Gelato DAI-USDC) will be onboarded in a future executive vote as the Protocol Engineering Core Unit's schedule allows. 
